...
## 项目描述
#### **本项目是个人工作多年的经验总结**
基于 **CocoaAsyncSocket** 实现的底层网络通信，项目为实际工作中生产环境下的即时通讯系统（**已脱敏**）。
...
- 能够分析 **TCP 长连接在即时通讯中的应用**，掌握 **心跳策略、连接管理、断线重连的最佳实践**。
...
### ✅ **已完成**
**NetworkManagerV2**：经过生产环境检测的**高性能、线程安全**的**单 TCP **通信模块，注释友好，方便单元测试，适用于小型项目以及中型项目早期。
- **连接管理**：单连接的建立与断开，支持 TLS  
- **粘包处理**：定长头部 + TLV 协议解析  
